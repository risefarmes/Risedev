...
local getcustomassets = {
	['Rise6/assets/rise/slice.png'] = 'rbxasset://risesix/slice.png',
	['Rise6/assets/rise/blur.png'] = 'rbxasset://risesix/blur.png',
	['Rise6/assets/new/blur.png'] = 'rbxassetid://14898786664',
}
...
local function addBlur(parent)
	...
	blur.Image = getcustomasset('Rise6/assets/new/blur.png')
...
local function writeFont()
	...
	writefile('Rise6/assets/rise/risefont.json', httpService:JSONEncode({
		name = 'ProductSans',
		faces = {
			{style = 'normal', assetId = getcustomasset('Rise6/assets/rise/SF-Pro-Rounded-Light.otf'), name = 'Light', weight = 300},
			{style = 'normal', assetId = getcustomasset('Rise6/assets/rise/SF-Pro-Rounded-Regular.otf'), name = 'Regular', weight = 400},
			{style = 'normal', assetId = getcustomasset('Rise6/assets/rise/SF-Pro-Rounded-Medium.otf'), name = 'Medium', weight = 500},
			{style = 'normal', assetId = getcustomasset('Rise6/assets/rise/Icon-1.ttf'), name = 'Icon1', weight = 600},
			{style = 'normal', assetId = getcustomasset('Rise6/assets/rise/Icon-3.ttf'), name = 'Icon3', weight = 800}
		}
	}))
	return getcustomasset('Rise6/assets/rise/risefont.json')
end

if inputService.TouchEnabled then
	writefile('Rise6/profiles/gui.txt', 'new')
	return
end
...
do
	...
	local res = isfile('Rise6/profiles/color.txt') and loadJson('Rise6/profiles/color.txt')
	if res then
				...
				if optionsettings.Profiles then
					...
					if ind then
						if val ~= 'default' then
							...
							if isfile('Rise6/profiles/'..val..mainapi.Place..'.txt') and delfile then
								delfile('Rise6/profiles/'..val..mainapi.Place..'.txt')
							end
						end
					else
...
function mainapi:Load(skipgui, profile)
	...
	if isfile('Rise6/profiles/'..game.GameId..'.gui.txt') then
		guidata = loadJson('Rise6/profiles/'..game.GameId..'.gui.txt')
		if not guidata then
			guidata = {Categories = {}}
	...
	if isfile('Rise6/profiles/'..self.Profile..self.Place..'.txt') then
		local savedata = loadJson('Rise6/profiles/'..self.Profile..self.Place..'.txt')
...
function mainapi:Save(newprofile)
	...
	writefile('Rise6/profiles/'..game.GameId..'.gui.txt', httpService:JSONEncode(guidata))
	writefile('Rise6/profiles/'..self.Profile..self.Place..'.txt', httpService:JSONEncode(savedata))
end
...
mainapi.Categories.Main:CreateDropdown({
	...
	Function = function(val, mouse)
		if mouse then
			writefile('Rise6/profiles/gui.txt', val)
			shared.vapereload = true
			if shared.VapeDeveloper then
				loadstring(readfile('Rise6/loader.lua'), 'loader')()
			else
				loadstring(game:HttpGet('https://raw.githubusercontent.com/risefarmes/Risedev/'..readfile('Rise6/profiles/commit.txt')..'/loader.lua', true))()
			end
		end
	end
})
		...
		for i, v in mainapi.Modules do
			if v.Enabled or table.find(found, i) then
				...
				if interfacebar.Enabled then
					...
					holderline.Image = getcustomasset('Rise6/assets/rise/slice.png')
